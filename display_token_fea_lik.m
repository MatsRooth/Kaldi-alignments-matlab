function display_token_fea_lik(tokenfile,datfile,feafile,likfile,framec,audiodir)
% Display word tokens, together with features and likes.
...
if nargin < 1
    ...
    likfile = '/local/res/stress/data/cvc_word_delta_SAME_t.lik';
...
positionVector1 = [0.05, 0.7, 0.9, 0.25];
positionVector2 = [0.05, 0.4, 0.9, 0.25];
positionVector3 = [0.05, 0.1, 0.9, 0.25];


% Features to display
Fea1 = 1:13;

% PDF ids we care about for SAME. These are zero-indexed.
PdfS = 64:66;
PdfEY1 = 28:30;
PdfM = 34:36;

Pdf1 = [PdfS,PdfEY1,PdfM];
...
% Structure encoding the likes for all tokens
Lik = readkalditfeatures(likfile);
% Map uid-k to matrix of features.
Lmap = feature_map(Lik);
...
% Set phone and audio data for k'th utterance.
    ...
    function display_alignment(f)
        ...
        for p = (F(1,F1) + 1):F(1,FN)
           ...
           transid = int2str(BASIC(k));
           text(k,ya *  (-0.7 + mod(p,2) * 0.08),pdfindex,'FontSize',12);
           text(k,ya *  (-0.6 + mod(p,2) * 0.08),transid,'FontSize',12);
        end
        ...
        % Features for the target word
        Fe1 = Fmap([uid,'-',int2str(wrdi)]);
        % Fea1 are the features we are interested in.
        Fe2 = Fe1(Fea1,:);
        % Graph them.
        imagesc([zeros(13,1),Fe2,zeros(13,1)]);
        
        % Plot the likes for the target word
        subplot('Position',positionVector3);
 
        Li1 = Lmap([uid,'-',int2str(wrdi)]);
        % This is not right, but it reveals some correlated pdf ids
        [Lval1 Lind1] = sort(Li1,'descend');
        disp(Lval1(1:10,:));
        disp(Lind1(1:10,:));
        Li1 = flipud(Li1);
        Li2 = Li1(Pdf2,:);
        mn = min(min(Li2));
        imagesc([mn * ones(9,1),Li2,mn * ones(9,1)]);
        %imagesc([mn * ones(40,1),Li2,mn * ones(40,1)]);
    end
